...
async function main() {
  console.log('🚀 เริ่มต้นการ Seed ตาราง person จำนวน 2,039 คน (คนเดียวกับ patient)...');
  ...
  // 2. Fetch existing patients from DB
  ...
  console.log(`✅ พบผู้ป่วยในตาราง patient: ${existingPatients.length} คน`);

  // 3. Prepare 2,039 person records
  ...
  for (let i = 0; i < TOTAL_TARGET; i++) {
    ...
    // Use existing patient if available
  ...
  for (let i = 0; i < personRecords.length; i += BATCH_SIZE) {
    ...
    process.stdout.write(`\r⏳ กำลังบันทึกข้อมูล: ${i + chunk.length}/${personRecords.length} (${percent}%) [สำเร็จ: ${success}, ผิดพลาด: ${fail}]`);
  }

  console.log(`\n\n🎉 บันทึกตาราง person เสร็จสิ้นใน ${((Date.now() - start) / 1000).toFixed(1)} วินาที`);
